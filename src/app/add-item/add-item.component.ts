...
export class AddItemComponent implements OnInit {
  ...
  validationContextClass: String = 'not-valid';
  ...
  onValueChanged() {
    this.validationClass = this.value.length > 5 ? 'valid' : 'not-valid';
  }

  onContentValueChanged() {
    this.validationContextClass = this.contentValue.length > 20 ? 'valid' : 'not-valid';
  }

  addTodo() {
    if (this.validationClass === 'not-valid' ||
      this.validationContextClass === 'not-valid') {
      return;
    }
  ...
  editTodo() {
    ...
    this.store.dispatch(new fromListActions.ChangeTitleAction(ALL_TODOS, this.index, this.value, this.contentValue));
    this.router.navigate(['/list']);
  }
  ...
  ngOnInit() {
    ...
    if (this.currentId && ALL_TODOS.length) {
      ...
      this.contentValue = ALL_TODOS[this.index].content;
      this.onValueChanged();
